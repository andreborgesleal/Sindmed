...
{
    public class AccountModel : ProcessContext<Associado, RegisterViewModel, ApplicationContext>
    {
        private ChamadoViewModel chamadoViewModel { get; set; }
        private RegisterViewModel registerViewModel { get; set; }

        #region Métodos da classe CrudContext
        public override Associado ExecProcess(RegisterViewModel value, Crud operation)
        {
            ...
            AssociadoModel associadoModel = new AssociadoModel(db);
            associadoModel.seguranca_db = this.seguranca_db;

            #region validar inclusão
            value.dt_cadastro = DateTime.Now.AddHours(_fuso_horario);
            value.mensagem = associadoModel.Validate(value, Crud.INCLUIR);
            ...
            if (u == null)
            {
                ...
                UsuarioRepository usuarioRepository = new UsuarioRepository()
                {
                    ...
                    dt_cadastro = DateTime.Now.AddHours(_fuso_horario),
                    situacao = _situacao,
                    isAdmin = "N",
                    senha = value.senha,
                    uri = value.uri,
            ...
            Associado entity = associadoModel.MapToEntity(value);
            #endregion

            entity = this.db.Set<Associado>().Add(entity);
            AssociadoViewModel associadoViewModel = associadoModel.MapToRepository(entity); // precisa mapear para recuperar o associadoID que foi incluído
            #endregion
            ...
            if (value.ind_proprietario == "S")
            {
                ...
                if (value.Dependentes.Where(info => info.tx_relacao_associado == "Cônjuge").Count() > 0)
                    _nome_conjuge = value.Dependentes.Where(info => info.tx_relacao_associado == "Cônjuge").FirstOrDefault().nome;
                ...
                ProprietarioViewModel proprietarioViewModel = new ProprietarioViewModel()
                {
                    ...
                    nome_conjuge = _nome_conjuge != null && _nome_conjuge.Length > 40 ? _nome_conjuge.Substring(0, 40) : _nome_conjuge,
                    endereco = entity.enderecoCom != null && entity.enderecoCom.Length > 50 ? entity.enderecoCom.Substring(0, 50) : entity.enderecoCom,
                    complemento = entity.complementoEndCom != null && entity.complementoEndCom.Length > 25 ? entity.complementoEndCom.Substring(0, 25) : entity.complementoEndCom,
                ...
                proprietarioViewModel = proprietarioModel.MapToRepository(proprietario); // precisa mapear para recuperar o associadoID que foi incluído
            }
            #endregion
            ...
            chamadoViewModel = new ChamadoViewModel()
            {
                ...
                chamadoMotivoId = 1, // Cadastro
                chamadoStatusId = 3, // Pendente de documento
        ...
        public override Validate AfterInsert(RegisterViewModel value)
        {
            ...
            int? _chamadoId = (from cham in db.Chamados.AsEnumerable()
                               where cham.associadoId == value.associadoId
            ...
            for (int i = 0; i <= _usuarioId.Where(info => info.HasValue).Count() - 1; i++)
            {
                AlertaRepository alerta = new AlertaRepository()
                {
                    ...
                    mensagemAlerta = "<b>" + DateTime.Now.AddHours(_fuso_horario).ToString("dd/MM/yyyy HH:mm") + "h</b><p>" + chamadoViewModel.assunto + "</p>"
                };
            ...
            if (db.Parametros.Find((int)DWM.Models.Enumeracoes.Enumeradores.Param.HABILITA_EMAIL).valor == "S")
            {
                ...
                if (result.Code > 0)
                {
                    result.MessageBase = "Seu cadastro foi realizado com sucesso, mas por falhas de comunicação não foi possível enviar seu e-mail de confirmação. Entretanto, você já pode acessar normalmente o sistema e visualizar o seu cadastro.";
        ...
        public override Associado MapToEntity(RegisterViewModel value)
        {
        ...
        public override RegisterViewModel MapToRepository(Associado entity)
        {
        ...
        public override Associado Find(RegisterViewModel key)
        {
        ...
        public override Validate Validate(RegisterViewModel value, Crud operation)
        {
            ...
            if (value.codigo_barra == null || value.codigo_barra.Replace(".", "").Replace(" ", "").Replace("-", "").Trim().Length != 47)
            {
                value.mensagem.Code = 5;
                value.mensagem.Message = MensagemPadrao.Message(5, "Código de Barras").ToString();
                value.mensagem.MessageBase = "Código de barras deve ser informado e deve possuir 47 dígitos numéricos.";
                return value.mensagem;
            }

            if (db.Unidades.Where(info => info.torreId == value.torreId && 
                                          info.unidadeId == value.unidadeId && 
                                          info.codigoBarra == value.codigo_barra.Replace(".", "").Replace(" ", "").Replace("-","")).Count() == 0)
            {
                value.mensagem.Code = 20;
                value.mensagem.Message = MensagemPadrao.Message(20).ToString();
                value.mensagem.MessageBase = "Código de barras digitado não corresponde ao código de barras da Unidade informada. Favor preencher o código de barras correspondente à Unidade " + value.apto + " referente vencimento abril/2014.";
                return value.mensagem;
            }
